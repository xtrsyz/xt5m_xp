...
function getFromDataBase( playerId, xPlayer )
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer then
			...
			if Config.Leaderboard.ShowPing then
				Player.ping = GetPlayerPing(xPlayers[i])
			end
...
AddEventHandler('esx:playerLoaded', getFromDataBase)
...
AddEventHandler("esx_xp:load", function()
	...
	if xPlayer then
		local CurrentXP = xPlayer.get("xp")
		local CurrentRank = xPlayer.get("rank")

		if CurrentRank and CurrentXP then
			if Config.Leaderboard.Enabled then
				TriggerClientEvent("esx_xp:init", _source, CurrentXP, CurrentRank, GetOnlinePlayers(_source, players))
			else
				TriggerClientEvent("esx_xp:init", _source, CurrentXP, CurrentRank, false)
			...
			getFromDataBase(_source, xPlayer)
		end
	end
end)
